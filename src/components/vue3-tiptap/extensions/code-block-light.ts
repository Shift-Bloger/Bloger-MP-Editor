	...
	renderHTML({ node }) {
		...
		return [
			...
			"section",
			{
				...
				"data-language": lang,
				// 外层 Mac 风格窗口内联样式
				style: "background-color: #1e1e1e; border-radius: 8px; margin: 20px 0; overflow: hidden; font-family: Consolas, Monaco, 'Andale Mono', 'Ubuntu Mono', monospace;"
			},
			[
				// 顶部状态栏 (红黄绿三个点 + 语言提示) - 避免使用 display: flex，微信会过滤
				"section",
				{
					style: "height: 32px; line-height: 32px; padding: 0 16px; background-color: #2d2d2d; display: block;"
				},
			...
			[
				...
				"pre",
				{
					...
					"data-lang": lang,
					style: "padding: 16px; margin: 0; overflow-x: auto; color: #abb2bf; font-size: 14px; line-height: 1.6; display: block; white-space: pre; background: #1e1e1e;"
				},
